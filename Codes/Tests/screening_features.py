...
####################################################################################################################################
# Screening features based on variation criteria:

# Class for screening continuous features:
class screening_continuous(object):
    """
    Arguments for initialization:
    	'features': list with names of numerical variables.
    	'num_continuous_feat': number (integer or float) of numerical variables to be selected based on the variance criterion (p*).
    ...
    """
    def __init__(self, features, num_continuous_feat, na_features = [], stat = 'variance',
        ...
        self.num_continuous_feats = num_continuous_feat
        self.stat = stat
    ...
    def select_feat(self, data):
        ...
        if self.stat == 'coef_variation':
            var_continuous_feat = pd.DataFrame(data={'feature': list(self.features),
                                               'variation': [coef_variation(data[f]) for f in self.features]})
        ...
        if self.collinearity:
            ...
            for f in list(self.var_continuous_feat.feature)[1:]:
                ...
                if len(self.selected_feat) < self.num_continuous_feats:
                    pass
                else:
                    break
...
class screening_categorical(object):
    """
    Arguments for initialization:
